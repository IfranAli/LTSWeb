...
export class FinanceSummaryService {
  ...
  private financesByCategory = rxResource({
    ...
    loader: ({ request: params }) => {
      ...
      return this.http.get<ApiFinanceSummary[]>(`${financesUrl}/summary/${this.$accountId}`, {
        ...getHttpHeaders(),
        params: {
  ...
  public financesData = computed(() => this.financesByCategory.value()?.flatMap((f) => f.items) ?? []);
  ...
  public summariesProcessed = computed(() => {
    ...
    const summaryGraphs: FinanceSummary[] = data
      .map((d): FinanceSummary => {
        ...
        const color = this.categoryService.categoryColorMap().get(categoryName) ?? '#A8D6D6';

        const items = d.items.map((i) => {
          const category = this.categoryService.categoryTypesMap().get(i.categoryType) ?? '';
          return {
            ...i,
