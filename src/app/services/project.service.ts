...
export class ProjectService {
  ...
  createProject(project: Omit<ProjectDatabaseModel, 'id'>) {
    const url = 'projects';
    return this.http.post<ProjectDatabaseModel[]>(baseUrl + url, project, getHttpHeaders());
  }
  ...
  createTaskOnProject(task: Partial<TaskModelPublic>) {
    ...
    return this.http.post<TaskDatabaseModel[]>(baseUrl + url, task, getHttpHeaders());
  }

  updateTask(task: Partial<TaskModel>) {
    const url = `tasks/${task.id}`;
    return this.http.put<TaskDatabaseModel[]>(baseUrl + url, task, getHttpHeaders());
  }
  ...
  updateProject(model: ProjectModel) {
    ...
    return this.http.put<ProjectDatabaseModel[]>(baseUrl + url, model, getHttpHeaders());
  }
}
